...
try {
    // Update ticket — approve only
    $upd = $conn->prepare(
        "UPDATE delivery_damage_tickets
         SET status = 'approved', reviewed_by = ?, reviewed_by_username = ?, reviewed_at = NOW(), admin_notes = ?
         WHERE id = ?"
    );
    $upd->bind_param("issi", $user_id, $username, $admin_notes, $ticket_id);
    $upd->execute();
