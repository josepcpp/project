...
// Load all pending_request batches
$pq = $conn->query(
    "SELECT rb.*, COUNT(ri.id) AS item_count,
     ...
     WHERE rb.status = 'pending_request'
     GROUP BY rb.id
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="card-modern p-8">
        <h3 class="serif-title text-xl font-black text-slate-800 mb-6">Batches Awaiting Validator Request</h3>

        <?php if (!$pq || $pq->num_rows === 0): ?>
            <p class="text-slate-400 text-sm font-bold text-center py-10">No batches pending. All clear.</p>
        <?php else: ?>
            ...
            <table class="table-modern w-full text-sm">
                ...
                <?php while ($b = $pq->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td class="font-bold"><?= htmlspecialchars($b['receiver_name'] ?? $b['receiver_username'] ?? '—') ?></td>
                        ...
                        <td>
                            <?php if (intval($b['item_count']) > 0): ?>
                            <a href="validator_request.php?batch_id=<?= $b['id'] ?>"
                            ...
                            <?php else: ?>
                            <span class="text-slate-400 text-xs font-bold">No items encoded yet</span>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endwhile; ?>
