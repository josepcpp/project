...
if ($action === 'procurement_approve' || $action === 'procurement_deny') {
    ...
    if ($action === 'procurement_deny') {
        ...
        $pal->bind_param("isiss", $id, $staff_uname, PROC_DENIED, $user_id, $admin_uname);
        $pal->execute();
    ...
    header("Location: suppliers.php?procurement_for=" . $id . "&uname=" . urlencode($staff_row['username']));
    exit();
}
